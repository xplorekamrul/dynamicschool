...
async function getCachedSeoContent(instituteId: string) {
  "use cache";
  return await getSeoContent(instituteId, "red-crescent");
}
...
export default async function RedCrescentPage() {
  const instituteId = await getInstituteId();
  ...
  const content = await getPageContent("red-crescent", instituteId);
  console.log("[Red Crescent Page] Content fetch result:", { slug: "red-crescent", instituteId, contentFound: !!content });
