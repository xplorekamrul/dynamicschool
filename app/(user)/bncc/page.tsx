...
import { debugLexicalStructure, lexicalToHtml } from "@/lib/shared/lexical-parser";
import Image from "next/image";


async function getCachedSeoContent(instituteId: string) {
   "use cache";
   return await getSeoContent(instituteId, "bncc");
}
...
export default async function BNCCPage() {
   ...
   const content = await getPageContent("bncc", instituteId);
   // console.log("[BNCC Page] Content fetch result:", { slug: "bncc", instituteId, contentFound: !!content });

   if (content?.body) {
      debugLexicalStructure(content.body);
   }
