...
async function getCachedSeoContent(instituteId: string) {
  "use cache";
  return await getSeoContent(instituteId, "regarding-admission");
}
...
export async function generateMetadata() {
  ...
  return {
    title: seoContent.title || "Regarding Admission",
...
export default async function RegardingAdmissionPage() {
  const instituteId = await getInstituteId();
  ...
  const content = await getPageContent("regarding-admission", instituteId);
      ...
      <section className="container mx-auto px-4 py-10 md:py-16">
        <article className="flex flex-col items-center">
          ...
          <div className="w-full max-w-3xl">
            ...
            {content.body && (
              <div
                className="text-base sm:text-lg text-gray-700 leading-relaxed md:leading-8 prose prose-sm max-w-none"
