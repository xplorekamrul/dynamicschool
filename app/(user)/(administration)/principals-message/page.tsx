...
async function getCachedSeoContent(instituteId: string) {
  "use cache";
  return await getSeoContent(instituteId, "principals-message");
}
...
async function PrincipalsMessageContent() {
  ...
  const content = await getPageContent("principals-message", instituteId);
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      <div className="bg-white shadow-xl rounded-2xl border border-gray-200 p-8 space-y-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          {imageUrl && (
            <Image
              ...
              height={500}
              src={imageUrl}
              alt={content.img_alt || "প্রধান শিক্ষক"}
            />
          )}
        ...
        <div className="pt-6 border-t border-gray-200 text-right">
          ...
          <p className="text-muted-foreground mt-1">প্রধান শিক্ষক</p>
        </div>
      </div>
    </div>
  );
